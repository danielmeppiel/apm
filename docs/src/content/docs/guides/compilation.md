...
description: "How APM's compiler transforms instruction packages into optimized, scoped AGENTS.md files."
---
...
> **Note**: `AGENTS.md` and `CLAUDE.md` contain **only instructions** (grouped by `applyTo` patterns). Prompts, agents, commands, hooks, and skills are integrated by `apm install`, not `apm compile`. See the [Integrations Guide](/apm/integrations/ide-tools/) for details on how `apm install` populates `.github/prompts/`, `.github/agents/`, `.github/skills/`, and `.claude/commands/`.
...
**Technical Implementation**: [`src/apm_cli/compilation/`](https://github.com/microsoft/apm/tree/main/src/apm_cli/compilation/)  
**Mathematical Core**: [`context_optimizer.py`](https://github.com/microsoft/apm/blob/main/src/apm_cli/compilation/context_optimizer.py)