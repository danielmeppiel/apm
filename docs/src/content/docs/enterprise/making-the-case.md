...
- **Enforce standards via CI gates.** `apm audit` gates pull requests via exit codes today (content scanning). Lockfile consistency checking (`apm audit --ci`) is planned, which will additionally catch manifest/lockfile drift.
...
- **CI enforcement.** There is no plugin equivalent of `apm audit` for content scanning in a CI pipeline (lockfile consistency checking via `--ci` is planned).
...
| CI enforcement | Custom scripts | Not available | `apm audit` exit codes (available); lockfile checking via `--ci` (planned) |
