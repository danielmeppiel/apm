...
3. Auto-detect from existing folders:
   - .github/ exists AND .claude/.opencode/ don't → copilot (internal: "vscode")
   - .claude/ exists AND .github/.opencode/ don't → claude
   - .opencode/ exists AND .github/.claude/ don't → opencode
...
UserTargetType = Literal[
    "copilot", "vscode", "agents", "claude", "opencode", "all", "minimal"
]
...
def detect_target(
    ...
    if github_exists and not claude_exists and not opencode_exists:
        return "vscode", "detected .github/ folder"
    elif claude_exists and not github_exists and not opencode_exists:
        return "claude", "detected .claude/ folder"
    elif opencode_exists and not github_exists and not claude_exists:
        return "opencode", "detected .opencode/ folder"
    elif github_exists or claude_exists or opencode_exists:
        return "all", "detected multiple target folders"
    else:
