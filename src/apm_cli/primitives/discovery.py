...
def scan_plugin_primitives(base_dir: str, collection: PrimitiveCollection) -> None:
    ...
    for owner_dir in apm_modules_path.iterdir():
        ...
        for repo_dir in owner_dir.iterdir():
            ...
            # Check if this is a plugin (has plugin.json at root or in plugins/ dir)
            plugin_json = repo_dir / "plugin.json"
            plugins_dir = repo_dir / "plugins"
            plugin_json_in_plugins = plugins_dir / "plugin.json"
            
            if not plugin_json.exists() and not plugin_json_in_plugins.exists():
                continue
            
            # Prefer plugin.json in plugins/ directory, fall back to root
            active_plugin_json = plugin_json_in_plugins if plugin_json_in_plugins.exists() else plugin_json
            ...
            # Plugins structure: all primitives are in plugins/ subdirectory
            if not plugins_dir.exists():
                continue
            
            # Scan plugins/agents/ (including subdirectories)
            ...
            # Scan plugins/skills/ subdirectories
            ...
            # Scan plugins/instructions/ (including subdirectories)
            instructions_dir = plugins_dir / "instructions"
            ...
            # Also check for SKILL.md in the plugins/ directory
            _discover_skill_in_directory(plugins_dir, collection, source)
