...
class SkillIntegrator(BaseIntegrator):
    ...
    def _integrate_native_skill(
        ...
        sub_skills_count, sub_deployed = self._promote_sub_skills(
            sub_skills_dir, github_skills_root, skill_name, warn=True, owned_by=owned_by
        )
        ...
        if claude_dir.exists() and claude_dir.is_dir():
            ...
            claude_skills_root = claude_dir / "skills"
            _, claude_sub_deployed = self._promote_sub_skills(
                sub_skills_dir, claude_skills_root, skill_name, warn=False
            )
            all_target_paths.extend(claude_sub_deployed)

        # OpenCode compatibility copy to .opencode/skills/ when .opencode/ exists
        opencode_dir = project_root / ".opencode"
        if opencode_dir.exists() and opencode_dir.is_dir():
            ...
            opencode_skills_root = opencode_dir / "skills"
            _, opencode_sub_deployed = self._promote_sub_skills(
