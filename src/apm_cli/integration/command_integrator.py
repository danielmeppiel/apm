...
class CommandIntegrator(BaseIntegrator):
    ...
    def _transform_prompt_to_command(self, source: Path) -> tuple:
        ...
        if "description" in post.metadata:
            command_metadata["description"] = post.metadata["description"]

        # OpenCode-specific fields
        if "agent" in post.metadata:
            command_metadata["agent"] = post.metadata["agent"]

        if "model" in post.metadata:
            command_metadata["model"] = post.metadata["model"]

        if "subtask" in post.metadata:
            command_metadata["subtask"] = post.metadata["subtask"]
        elif "argument-hint" in post.metadata:
            command_metadata["subtask"] = post.metadata["argument-hint"]
        elif "argumentHint" in post.metadata:
            command_metadata["subtask"] = post.metadata["argumentHint"]

        # Create new post with OpenCode metadata
        new_post = frontmatter.Post(post.content)
        new_post.metadata = command_metadata
    ...
    def integrate_command(
        ...
        Args:
            ...
            target: Target command file path in .opencode/commands/
    ...
    def sync_integration(
        self, apm_package, project_root: Path, managed_files: set = None
    ) -> Dict:
    ...
    def remove_package_commands(
        self, package_name: str, project_root: Path, managed_files: set = None
    ) -> int:
