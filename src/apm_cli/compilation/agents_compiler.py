...
class CompilationConfig:
    ...
    # "vscode" or "agents" or "opencode" -> AGENTS.md + .github/
    ...
    exclude: List[str] = (
        None  # Glob patterns for directories to exclude during compilation
    )
        ...
        try:
            ...
            if Path('apm.yml').exists():
                ...
                if 'min_instructions_per_file' in placement_config:
                    config.min_instructions_per_file = placement_config[
...
class AgentsCompiler:
    ...
    def compile(
        ...
        - "vscode" or "agents" or "opencode": Generate AGENTS.md + .github/ structure
    ...
    def _compile_distributed(
        ...
        if compilation_results:
            if config.debug or config.trace:
                ...
                output = distributed_compiler.output_formatter.format_verbose(
                    compilation_results
                )
            elif config.dry_run:
                # Dry run mode with placement preview
                output = distributed_compiler.output_formatter.format_dry_run(
                    compilation_results
                )
            else:
                # Default mode with essential information
                output = distributed_compiler.output_formatter.format_default(
                    compilation_results
                )
    ...
    def _compile_claude_md(
        ...
        for placement in claude_result.placements:
            ...
            summary_lines.append(
                f"- {rel_path} ({len(placement.instructions)} instructions)"
            )
    ...
    def generate_output(
        self, template_data: TemplateData, config: CompilationConfig
    ) -> str:
    ...
    def _write_distributed_file(
        ...
        try:
            ...
            raise OSError(
                f"Failed to write distributed AGENTS.md file {agents_path}: {str(e)}"
            )
    ...
    def _display_trace_info(
        self, distributed_result, primitives: PrimitiveCollection
    ) -> None:
        ...
        for placement in distributed_result.placements:
            ...
            for instruction in placement.instructions:
                ...
                try:
                    inst_path = instruction.file_path.relative_to(
    ...
    def _generate_placement_summary(self, distributed_result) -> str:
        ...
        for placement in distributed_result.placements:
            ...
            lines.append(
                f"   Patterns: {', '.join(sorted(placement.coverage_patterns))}"
            )
            lines.append("")
    ...
    def _generate_distributed_summary(
        self, distributed_result, config: CompilationConfig
    ) -> str:
