...
class TestCodexDockerArgsFix:
            ...
            "packages": [
                {
                    ...
                    "version": "latest",
                    "runtime_arguments": [
                        {"format": "string", "is_required": True, "type": "positional", "value": "run", "value_hint": "docker_cmd"},
                        {"format": "string", "is_required": True, "type": "named", "value": "-i", "value_hint": "interactive_flag"},
                        {"format": "string", "is_required": True, "type": "named", "value": "--rm", "value_hint": "remove_flag"},
                        {"format": "string", "is_required": True, "type": "named", "value": "-e", "value_hint": "env_flag"},
                        {"format": "string", "is_required": True, "type": "positional", "value": "GITHUB_PERSONAL_ACCESS_TOKEN", "value_hint": "env_var_name"},
                        {"format": "string", "is_required": True, "type": "positional", "value": "ghcr.io/github/github-mcp-server", "value_hint": "image"}
                    ],
                    ...
                    "environment_variables": [
                        ...
                        {"name": "GITHUB_HOST", "description": "GitHub Enterprise Server or ghe.com hostname (optional)"},
            ...
            "packages": [
                {
                    ...
                    "version": "1.8.1",
                    "runtime_arguments": [
                        {"format": "string", "type": "positional", "value": "-y", "value_hint": "noninteractive_mode"},
                        {"format": "string", "type": "positional", "value": "@notionhq/notion-mcp-server@1.8.1", "value_hint": "notion_mcp_version"},
                        {"format": "string", "type": "named", "value": "--transport", "value_hint": "transport_flag"},
                        {"format": "string", "type": "positional", "value": "stdio", "value_hint": "stdio"},
                        {"format": "string", "type": "named", "value": "--port", "value_hint": "port_flag"},
                        {"format": "string", "type": "positional", "value": "3000", "value_hint": "3000"},
                        {"format": "string", "is_required": True, "type": "positional", "value": "<NOTION_TOKEN>", "value_hint": "notion_token"}
                    ],
                    "package_arguments": [],
                    "environment_variables": [
                        {"name": "NOTION_TOKEN", "description": "Notion API token for authentication"}
                    ]
                }
            ]
        }
    ...
    def test_github_docker_server_config_generation(self, codex_adapter, github_mcp_server_data, sample_env_overrides):
        ...
        with patch.object(codex_adapter, 'registry_client') as mock_registry:
            ...
            result = codex_adapter.configure_mcp_server(
                ...
                env_overrides=sample_env_overrides
            )
            ...
            expected_env_vars = {"GITHUB_PERSONAL_ACCESS_TOKEN", "GITHUB_TOOLSETS", "GITHUB_READ_ONLY"}
            actual_env_vars = set()
            ...
            assert expected_env_vars.issubset(actual_env_vars), f"Missing env vars in args. Expected: {expected_env_vars}, Found: {actual_env_vars}"
    ...
    def test_notion_npm_server_config_generation(self, codex_adapter, notion_mcp_server_data, sample_env_overrides):
        ...
        with patch.object(codex_adapter, 'registry_client') as mock_registry:
            ...
            expected_args = ["-y", "@notionhq/notion-mcp-server@1.8.1", "--transport", "stdio", "--port", "3000", "secret_notion_token_67890"]
    ...
    def test_docker_server_with_package_arguments(self, codex_adapter, sample_env_overrides):
            ...
            "packages": [
                {
                    ...
                    "environment_variables": [
                        {"name": "TEST_TOKEN", "description": "Test token"}
                    ]
                }
            ]
        }
        ...
        with patch.object(codex_adapter, 'registry_client') as mock_registry:
            ...
            expected_args = ["run", "-i", "--rm", "example/test-server", "--verbose", "--config", "-e", "TEST_TOKEN", "/app/config.json"]
            assert server_config["args"] == expected_args
            
            # Environment variables should be in env section
            assert server_config["env"]["TEST_TOKEN"] == "test_token_value"

    def test_no_duplication_in_complex_scenarios(self, codex_adapter, github_mcp_server_data, sample_env_overrides):
        ...
        with patch.object(codex_adapter, 'registry_client') as mock_registry:
            ...
            expected_args = ["run", "-i", "--rm", "-e", "GITHUB_PERSONAL_ACCESS_TOKEN", "-e", "GITHUB_DYNAMIC_TOOLSETS", "-e", "GITHUB_READ_ONLY", "-e", "GITHUB_TOOLSETS", "ghcr.io/github/github-mcp-server"]
            assert server_config["args"] == expected_args
            ...
            assert args.count("-e") == 4  # One for each environment variable (including GITHUB_DYNAMIC_TOOLSETS default)
    ...
    def test_all_collected_env_vars_become_docker_flags(self, codex_adapter, github_mcp_server_data):
        ...
        with patch.object(codex_adapter, 'registry_client') as mock_registry:
            ...
            result = codex_adapter.configure_mcp_server(
                "io.github.github/github-mcp-server",
                env_overrides=comprehensive_env_overrides
            )
            ...
            assert expected_env_vars == actual_env_flags, f"Missing env flags. Expected: {expected_env_vars}, Got: {actual_env_flags}"
