...
pytestmark = pytest.mark.skipif(
    not E2E_MODE, 
    reason="MCP registry E2E tests only run when APM_E2E_TESTS=1 is set"
)
...
class TestMCPRegistryE2E:
    ...
    def test_registry_installation_with_codex(self, temp_e2e_home, apm_binary):
        ...
        with tempfile.TemporaryDirectory() as project_workspace:
            ...
            result = run_command(f"{apm_binary} init registry-test-project --yes", cwd=project_workspace)
            ...
            apm_config = {
                ...
                'scripts': {
                    'start': 'codex run start --param name="$name"'
                }
            }
    ...
    def test_empty_string_handling_e2e(self, temp_e2e_home, apm_binary):
        ...
        with tempfile.TemporaryDirectory() as project_workspace:
            ...
            if codex_config.exists():
                ...
                assert "GITHUB_TOKEN=" not in config_content, "Config should not contain empty env vars"
                assert 'GITHUB_TOKEN=""' not in config_content, "Config should not contain empty quoted env vars"
    ...
    def test_duplication_prevention_e2e(self, temp_e2e_home, apm_binary):
        ...
        with tempfile.TemporaryDirectory() as project_workspace:
            ...
            assert "already" in output2 or "nothing to install" in output2 or "configured" in output2, \
            ...
            if codex_config.exists():
                ...
                assert server_section_count == 1, f"GitHub server should appear exactly once, found {server_section_count} times"
                
                # Check that Docker args don't have duplicated configurations  
                import toml
                config_data = toml.loads(config_content)
                if "mcp_servers" in config_data and "github-mcp-server" in config_data["mcp_servers"]:
