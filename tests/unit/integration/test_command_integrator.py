...
""")
    ...
    def test_opencode_metadata_mapping(self, temp_project):
        """Test that only OpenCode-supported frontmatter fields are mapped."""
...
""")
        ...
        target = temp_project / ".opencode" / "commands" / "cmd-apm.md"
        ...
        assert post.metadata["agent"] == "helper-agent"
        assert post.metadata['model'] == 'claude-sonnet'
        assert post.metadata["subtask"] is True
