...
class TestMCPConflictDetection(unittest.TestCase):
    ...
    def test_detects_canonical_name_match(self):
        ...
        def mock_find_server(server_ref):
            if server_ref == "github":
                return {'name': 'io.github.github/github-mcp-server', 'id': 'github-server-uuid-123'}
            elif server_ref == "my-github-server":
                return {'name': 'io.github.github/github-mcp-server', 'id': 'github-server-uuid-123'}
            elif server_ref == "github-server":
                return {'name': 'io.github.github/github-mcp-server', 'id': 'github-server-uuid-123'}
            return None
    ...
    def test_allows_different_servers(self):
        ...
        def mock_find_server(server_ref):
            if server_ref == "notion":
                return {'name': 'io.github.makenotion/notion-mcp-server', 'id': 'notion-server-uuid-456'}
    ...
    def test_handles_registry_lookup_failure(self):
        ...
        self.mock_adapter.registry_client.find_server_by_reference.side_effect = Exception("Registry unavailable")
