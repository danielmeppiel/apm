...
class TestSiblingDirectoryCoverage:
    ...
    def sibling_directory_project(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (temp_path / "src" / "components" / "ContactForm.tsx").write_text("// ContactForm component")
            ...
            for dir_in_chain in inheritance_chain:
                if dir_in_chain in placement_map:
                    for instruction in placement_map[dir_in_chain]:
                        if (instruction.name == "react-components-local" and 
                            optimizer._is_instruction_relevant(instruction, sibling_dir)):
                            has_coverage = True
                            break
                if has_coverage:
                    break
    ...
    def test_minimal_coverage_for_sibling_directories(self, sibling_directory_project):
        ...
        root_dir = sibling_directory_project
        ...
        for tsx_dir in directories_with_tsx:
            ...
            for dir_in_chain in inheritance_chain:
                if dir_in_chain in placement_map:
                    for instruction in placement_map[dir_in_chain]:
                        if (instruction.name == "react-components" and
                            optimizer._is_instruction_relevant(instruction, tsx_dir)):
                            has_coverage = True
                            break
                if has_coverage:
                    break
