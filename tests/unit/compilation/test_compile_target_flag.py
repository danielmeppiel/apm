...
""")
    ...
    def test_target_opencode_routes_to_agents_compilation_path(
        self, temp_project, sample_primitives
    ):
        """Test that target='opencode' compiles through AGENTS.md path."""
        config_vscode = CompilationConfig(
            target="vscode", dry_run=True, single_agents=True
        )

        config_opencode = CompilationConfig(
            target="opencode", dry_run=True, single_agents=True
        )

        compiler = AgentsCompiler(str(temp_project))

        result_vscode = compiler.compile(config_vscode, sample_primitives)
        result_opencode = compiler.compile(config_opencode, sample_primitives)

        assert result_vscode.success == result_opencode.success
        assert "AGENTS.md" in result_vscode.output_path
        assert "AGENTS.md" in result_opencode.output_path
...
""")
    ...
    def test_target_opencode_shows_opencode_specific_compile_message(
        self, runner, temp_project
    ):
        """Test that opencode target message is OpenCode-specific."""
        original_dir = os.getcwd()
        try:
            os.chdir(temp_project)
            result = runner.invoke(
            ...
            assert "Compiling for AGENTS.md (OpenCode)" in result.output
        finally:
            os.chdir(original_dir)
...
""")
    ...
    def test_target_opencode_from_apm_yml_maps_to_agents_semantics(
        self, temp_project_with_config
    ):
        """Test that apm.yml target opencode compiles AGENTS target path."""
        apm_yml = temp_project_with_config / "apm.yml"
        apm_yml.write_text("""
name: test-project
version: 0.1.0
...
""")
        ...
        try:
            ...
            compiler = AgentsCompiler(str(temp_project_with_config))
            primitives = PrimitiveCollection()
            instruction = Instruction(
                name="test",
                file_path=temp_project_with_config
            ...
            (temp_project_with_config / ".apm" / "instructions").mkdir(
...
""")
            ...
            result = compiler.compile(
                CompilationConfig(
                    target=config.target, dry_run=True, single_agents=True
                ),
                primitives,
            )
            assert result.success
