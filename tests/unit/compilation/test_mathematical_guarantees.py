...
class TestMathematicalGuarantees:
    ...
    def test_hierarchical_inheritance_chain(self):
        ...
        assert self.base_path.resolve() in [p.resolve() for p in inheritance_chain], "Root not in inheritance chain"
    ...
    def test_no_data_loss_constraint(self):
        ...
        # Verify no data loss
        assert total_placed == len(instructions), f"Data loss detected: {len(instructions)} input, {total_placed} placed"
